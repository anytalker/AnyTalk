...
from modules.model import MyGeneratorFullModel, DiscriminatorFullModel
...
from frames_dataset import DatasetRepeater
                    # 使用NaN过滤器
def filter_nan(loss):
    return torch.where(torch.isnan(loss), torch.zeros_like(loss), loss)
...
def train(config, generator, discriminator, kp_detector, he_estimator, checkpoint, log_dir, dataset, device_ids):
    ...
    generator_full = MyGeneratorFullModel(kp_detector, he_estimator, generator, discriminator, train_params, estimate_jacobian=config['model_params']['common_params']['estimate_jacobian'])
            ...
            with tqdm(total=total) as par:
                for x in dataloader:
                    ...
                    for val in losses_generator.values():
                        filtered_loss = filter_nan(val)
                        if filtered_loss.shape[0]==0:
                            loss_values.append(torch.zeros((1,)))
                        else:
                            loss_values.append(filtered_loss.mean())
                    # loss_values = [val.mean() for val in losses_generator.values()]
                    loss = sum(loss_values)
                    if torch.isnan(loss) is None:
                        break
                    loss.backward()
                    ...
                    if train_params['loss_weights']['generator_gan'] != 0:
                        ...
                        for val in losses_discriminator.values():
                            ...
                            if filtered_loss.shape[0]==0:
                                ...
                                loss_values.append(filtered_loss.mean())
                        # loss_values = [val.mean() for val in ]
                        loss = sum(loss_values)
                        loss.backward()
                    ...
                    #     filtered_loss = filter_nan(value)
                    #     if filtered_loss.shape[0]==0:
                    #         losses[key]= 0
                    #     else:
                    #         losses[key]= filtered_loss.mean().detach().data.cpu().numpy() # loss_values.append(filtered_loss.mean())
