...
from modules.util import create_emo_loss
import torchvision
...
def strength_keypoint_transformation(kp_canonical, he):
    kp = kp_canonical['value']    # (bs, k, 3)
    yaw, pitch, roll = he['yaw'], he['pitch'], he['roll']
    t, exp = he['t'], he['exp']
    ...
    exp = exp.view(exp.shape[0], -1, 3)
    kp_transformed = kp_t + exp
    strength = kp_canonical['strength'] # (bs, k ,3, 3)
    return {'value': kp_transformed, 'strength': strength}
...
class GeneratorFullModel(torch.nn.Module):
    ...
    def forward(self, x,epoch=None):
        ...
        if self.loss_weights['headpose'] != 0:
            transform_hopenet =  torchvision.transforms.Compose([torchvision.transforms.Resize(size=(224, 224)),
        ...
        if self.loss_weights['expression'] != 0:
            if epoch is not None and epoch>=100:
                expression_wight = self.loss_weights['expression'] - (epoch-100)//50 - 2
            else:
                expression_wight = self.loss_weights['expression']
            expression_wight = max(expression_wight, 1 )
            value = torch.norm(he_driving['exp'], p=1, dim=-1).mean()  
            loss_values['expression'] = expression_wight * value
...
###########
class MyGeneratorFullModel(torch.nn.Module):
    """
    Merge all generator related updates into single model for better multi-gpu usage
    """

    def __init__(self, kp_extractor, he_estimator, generator, discriminator, train_params, estimate_jacobian=True):
        super(MyGeneratorFullModel, self).__init__()
        self.kp_extractor = kp_extractor
        self.he_estimator = he_estimator
        self.generator = generator
        self.discriminator = discriminator
        self.train_params = train_params
        #device, trainable=False, normalize_features=False, emo_feat_loss="mse_loss")
        ...
        if torch.cuda.is_available():
            self.pyramid = self.pyramid.cuda()
            print("load emo model to calcuate loss")
            self.emo_loss = create_emo_loss()
    ...
    def forward(self, x,epoch=None):
        ...
        if self.loss_weights['emotion_loss_weight'] !=0: #compute_loss(self, gt, predicted),
            value = self.emo_loss.compute_loss(x['driving'], generated['prediction'])
            loss_values['emotion_feature_matching'] = self.loss_weights['emotion_loss_weight'] * value
...
###########
class AttnGeneratorFullModel(torch.nn.Module):
    """
    Merge all generator related updates into single model for better multi-gpu usage
    """

    def __init__(self, kp_extractor, he_estimator, generator, discriminator, train_params, estimate_jacobian=False):
        super(AttnGeneratorFullModel, self).__init__()
        self.kp_extractor = kp_extractor
        self.he_estimator = he_estimator
        self.generator = generator
        self.discriminator = discriminator
    ...
    def forward(self, x):
        kp_canonical = self.kp_extractor(x['source'])     # {'value': value}
        he_source = self.he_estimator(x['source'])        # {'yaw': yaw, 'pitch': pitch, 'roll': roll, 't': t, 'exp': exp}
        he_driving = self.he_estimator(x['driving'])      # {'yaw': yaw, 'pitch': pitch, 'roll': roll, 't': t, 'exp': exp}

        # {'value': value, 'jacobian': jacobian}
        kp_source = strength_keypoint_transformation(kp_canonical, he_source)
        kp_driving = strength_keypoint_transformation(kp_canonical, he_driving)
        ...
        if (self.loss_weights['equivariance_value']) != 0: # + self.loss_weights['equivariance_jacobian']
            transform = Transform(x['driving'].shape[0], **self.train_params['transform_params'])
            transformed_frame = transform.transform_frame(x['driving'])

            transformed_he_driving = self.he_estimator(transformed_frame)

            transformed_kp = strength_keypoint_transformation(kp_canonical, transformed_he_driving)
