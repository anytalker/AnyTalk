...
from modules.util import KPHourglass, make_coordinate_grid, AntiAliasInterpolation2d, ResBottleneck,DownBlock3d
class StrengthKPDetector(nn.Module):
    """
    Detecting canonical keypoints. Return keypoint position and jacobian near each keypoint.
    """

    def __init__(self, block_expansion, feature_channel, num_kp, image_channel, max_features, reshape_depth,
                 num_blocks, temperature, estimate_jacobian=False, scale_factor=1, single_jacobian_map=False):
        super(StrengthKPDetector, self).__init__()
        reshape_channel = 1024 *  reshape_depth
        ...
        input_dim = [self.predictor.out_filters,64,128,192]
        output_dim = [64,128,192,256]
        strength_blocks = []
        for i in range(len(input_dim)):
            strength_blocks.append(DownBlock3d(input_dim[i], output_dim[i]))
        self.strength_blocks = nn.ModuleList(strength_blocks)
        self.strength = nn.Conv2d(256*reshape_depth, out_channels=num_kp*reshape_depth, kernel_size=4, padding=0)
        self.num_kp = num_kp
    ...
    def forward(self, x):
        ...
        for i in range(len(self.strength_blocks)):
            feature_map = self.strength_blocks[i](feature_map)
        
        # print(feature_map.shape) #torch.Size([3, 256, 16, 4, 4])
        bs,c,d,w,h= feature_map.shape
        strength = F.sigmoid(self.strength(feature_map.view(bs,-1,w,h))).view(bs,self.num_kp,d,1,1)
        
        # print("strength", strength.shape)
...
class KPDetector(nn.Module):
    ...
    def __init__(self, block_expansion, feature_channel, num_kp, image_channel, max_features,reshape_depth,reshape_channel,
                 num_blocks, temperature, estimate_jacobian=False, scale_factor=1, single_jacobian_map=False):
