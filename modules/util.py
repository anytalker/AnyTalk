...
import re
from emonet.Metrics import get_metric
from emonet.BarlowTwins import BarlowTwinsLossHeadless, BarlowTwinsLoss

# def _compute_emotion_loss(images, predicted_images, loss_dict, prefix,emonet_weight=1, va=None, expr7=None, with_grad=True, batch_size=None, ring_size=None):
#     emonet_loss = {}
...
#     emonet_loss["feat_2_L1"] = emo_feat_loss_2 * emonet_weight

def create_emo_loss(trainable=False, normalize_features=False, emo_feat_loss="mse_loss"):
    return EmoNetLoss( trainable=trainable, normalize_features=normalize_features, emo_feat_loss=emo_feat_loss)

def get_emonet(load_pretrained=True):
    from emonet.models import EmoNet

    n_expression = 8

    # Create the model
    net = EmoNet(n_expression=n_expression).cuda()

    # if load_pretrained:
    state_dict_path = Path(f'./emonet/emonet_{n_expression}.pth')
