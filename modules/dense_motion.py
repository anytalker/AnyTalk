...
import torch
from modules.util import Hourglass, make_coordinate_grid, kp2gaussian, AntiAliasInterpolation2d
from modules.util import  SameBlock2d
from sync_batchnorm import SynchronizedBatchNorm3d as BatchNorm3d
import math
from einops import rearrange, repeat
from torch import nn, einsum
class ImgWarpNet(nn.Module):
    """
    Module that predicting a dense motion from sparse motion representation given by kp_source and kp_driving
    """
    def __init__(self, scale_factor=0.25, reshape_depth = 4, **args):
        super(ImgWarpNet, self).__init__()
        self.scale_factor = scale_factor
        scale_times = 1//self.scale_factor
        self.feat_ch = int(64*scale_times)
        self.reshape_depth = reshape_depth       
        self.first = SameBlock2d(3, self.reshape_depth*3,  kernel_size=(3, 3), padding=(1, 1))

        self.fc_loc = nn.Sequential(
            nn.Linear(self.feat_ch*2+3, self.feat_ch),
            nn.ReLU(True),
            nn.Linear(self.feat_ch, self.feat_ch),
            nn.ReLU(True),
        )
        if self.scale_factor != 1:
            self.down = AntiAliasInterpolation2d(3, self.scale_factor)

    def forward(self, Query, Key, Value, image, **kwargs):
        if self.scale_factor != 1:
            image = self.down(image) 
        out = {}
        bs,c,h,w = Query.shape
        Query = rearrange(Query, 'b c x y -> b (x y) c')
        Key = rearrange(Key, 'b c x y -> b (x y) c')
        Value = rearrange(Value, 'b c x y -> b (x y) c')
        pixel = rearrange(image, 'b c x y -> b (x y) c')
        
        scale = Query.shape[1] ** -0.5
        dots = einsum('b i d, b j d -> b i j', Query, Key) * scale #(bz,w*h,w*h)
        out['motion'] = dots
        attn = dots.softmax(dim=-1) #(bz,w*h,w*h)

        t_out = einsum('b i j, b j d -> b i d', attn, Value) # (bz,w*h,w*h)

        mid = torch.cat((pixel,Key),-1) # b, w*h, c+3
        dot_mid = einsum('b i j, b j d -> b i d', attn, mid) #b, w*h, c+3
        mid = torch.cat(( Query, dot_mid),-1) # b, num, c+3+c
        mid = self.fc_loc(mid)
        t_out = t_out + mid
        out = rearrange(t_out, 'b (x y) c-> b c x y', x =h )
        return out
